  ...
  onStart: async function ({ api, event, args, message }) {
    ...
    if (args.length !== 1 || !categories.includes(args[0].toLowerCase())) {
      return message.reply(
        `Please specify a valid category. Available categories: ${categories.join(
          ", "
        )}`
      );
    }
    try {
      ...
      const response = await axios.get(
        `https://anivoice-mdlx.onrender.com/kshitiz/${category}`,
      ...
      if (error.response && error.response.status === 400) {
        ...
        message.reply(
          "Sorry, it seems like the API is having trouble. Please try again later."
        );
      } else {
        // Handle other errors
